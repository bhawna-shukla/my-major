...
const Managing = () => {
  ...
  const fetchProduct = async () => {
    const res = await fetch("http://localhost:5000/services/getbycategory/managing");
  ...
  const displayProduct = () => {
    return product.map((obj) => (
      <div className=" "  >
        ...
        <Link
          href={"/view/" + obj._id}
          className="group relative block bg-black rounded-xl  border border-gray-400 py-9"
        >
        
          <img
            alt=""
            src={obj.image}
            className="absolute   w-full object-cover opacity-75 transition-opacity group-hover:opacity-50"
            />
  ...
  <div className="overflow-hidden bg-white dark:bg-sky-800 lg:mx-8 lg:flex lg:max-w-6xl lg:w-full lg:shadow-md lg:rounded-xl">
    <div className="lg:w-1/2">
      <div
        ...
        style={{
          backgroundImage:
            'url("https://media.licdn.com/dms/image/C4E12AQGPKcRQN5iydw/article-cover_image-shrink_720_1280/0/1584727351803?e=2147483647&v=beta&t=NaZkKptFDTlpFhZ6RyrD0mQWvbwIB7s70N_zhHbQ3oc")'
        }}
      />
    </div>
